...
function OrgCard({ member, featured = false, delay = 0 }: { member: OrgMember; featured?: boolean; delay?: number }) {
  return (
    ...
    >
      <h3 className={`${featured ? 'text-xl' : 'text-lg'} font-bold text-white mb-1`}>{member.role}</h3>
...
export function OrgStructure() {
  const [lead, platformManager, applicationManager, dataEngineer, devOpsEngineer] = orgStructureData;
